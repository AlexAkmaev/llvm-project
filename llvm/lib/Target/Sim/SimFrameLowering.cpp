...
void SimFrameLowering::RegAdjustment(MachineBasicBlock &MBB,
                                         MachineBasicBlock::iterator MBBI,
                                         int NumBytes,
                                         MachineInstr::MIFlag Flag,
                                         Register src, Register dest) const {
  DebugLoc dl;
  const SimInstrInfo &TII = *static_cast<const SimInstrInfo *>(ST.getInstrInfo());
  if (!isInt<16>(NumBytes)) {
    llvm_unreachable("Invalid SP adjistment"); 
  }
...
void SimFrameLowering::emitPrologue(MachineFunction &MF,
    ...
    RegAdjustment(MBB, MBBI, -NumBytes,
    ...
    NumBytes -= tmp / 4;
    // Set new FP value
    RegAdjustment(MBB, MBBI, NumBytes,MachineInstr::FrameSetup,SIM::SP, SIM::FP);
}
...
void SimFrameLowering::emitEpilogue(MachineFunction &MF,
    ...
    RegAdjustment(MBB, MBBI, NumBytes, MachineInstr::FrameDestroy, SIM::SP, SIM::SP);
...
static bool isFrameIndexInCalleeSavedRegion(const MachineFrameInfo &MFI, int FI) {
...
StackOffset SimFrameLowering::getFrameIndexReference(const MachineFunction &MF, int FI, Register &FrameReg) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
  const SimRegisterInfo *RegInfo = ST.getRegisterInfo();

  int FrameOffset = MFI.getObjectOffset(FI) - getOffsetOfLocalArea() + MFI.getOffsetAdjustment();
...
MachineBasicBlock::iterator SimFrameLowering::eliminateCallFramePseudoInstr(MachineFunction &MF, MachineBasicBlock &MBB,
  ...
  if (!hasReservedCallFrame(MF)) {
    ...
    if (Size) {
      RegAdjustment(MBB, I, Size / 4, MachineInstr::NoFlags, SIM::SP, SIM::SP);
    }
  }
  return MBB.erase(I);
}
...
void SimFrameLowering::determineCalleeSaves(MachineFunction &MF, BitVector &SavedRegs, RegScavenger *RS) const {
